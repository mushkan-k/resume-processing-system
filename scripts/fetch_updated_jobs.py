"""
Fetch all updated job records from JobDiva (NewUpdatedJobRecords) in 14-day intervals over the last 2 years.
Saves results as JSON files with max 100 jobs per file.

Output: data/updated_jobs/jobs_001.json, jobs_002.json, ...
"""
import os
import json
import requests
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "updated_jobs")
JOBS_PER_FILE = 100
...
def fetch_chunk(token, from_date, to_date):
    """Fetch updated job records for a 14-day window."""
    headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}
    params = {
        "fromDate": from_date.strftime("%m/%d/%Y %H:%M:%S"),
        "toDate": to_date.strftime("%m/%d/%Y %H:%M:%S"),
    }
    resp = requests.get(
...
def main():
    print("=" * 60)
    print("FETCH UPDATED JOB RECORDS - Last 4 Years (14-day chunks)")
    print("=" * 60)
    ...
    for job in all_jobs:
        ...
        seen[job_id] = job  # later entries overwrite earlier ones
