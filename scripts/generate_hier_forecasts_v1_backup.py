...
def build_panel(df, jolts, unrate, spread_m, sent):
    """Build monthly panel with lag features + macro signals."""
    ...
    # Viable combos: >=20 months of activity
...
def fit_and_predict(panel, n_roles, n_comps, n_combos, forecast_months):
    ...
    feat_cols = ['lag1', 'lag2', 'lag3', 'roll3',
    ...
    print(f"\n  Fitting hierarchical model ({NUM_CHAINS} chains × {NUM_SAMPLES} samples)...")
    ...
    mcmc.run(jax.random.PRNGKey(42),
             role_idx=jnp.array(role_train),
             comp_idx=jnp.array(comp_train),
             combo_idx=jnp.array(combo_train),
             X=jnp.array(X_train_s),
             y=jnp.array(y_train_log),
             n_roles=n_roles, n_comps=n_comps, n_combos=n_combos)
    
    print("  Model fitted. Computing per-cluster accuracy...")
    
    # Compute per-cluster MAPE from in-sample fitted values
    ...
    # Static-lag forecast: use last known features for all months
    # (AR is unstable; static is robust for a 6-month horizon)
    X_pred = combos[feat_cols].values.astype(np.float32)
    X_pred_s = (X_pred - X_mean) / X_std
    
    role_pred = combos['role_idx'].values.astype(np.int32)
    comp_pred = combos['comp_idx'].values.astype(np.int32)
    combo_pred = combos['combo_idx'].values.astype(np.int32)
    X_pred_jax = jnp.array(X_pred_s)
    
    # Posterior mean forecast (no observation noise = tighter, more useful intervals)
    ...
    # Compute monthly seasonal indices per combo from training data
    # (ratio of each calendar month's avg demand to overall combo avg)
    panel_seasonal = panel.copy()
    panel_seasonal['cal_month'] = panel_seasonal['month'].dt.month
    combo_overall_avg = panel_seasonal.groupby('combo')['openings'].mean()
    combo_month_avg = panel_seasonal.groupby(['combo', 'cal_month'])['openings'].mean()
    
    # Build seasonal factor lookup: combo -> {cal_month: factor}
    seasonal_factors = {}
    for (combo_key, cal_m), avg_val in combo_month_avg.items():
        overall = combo_overall_avg.get(combo_key, avg_val)
        factor = avg_val / overall if overall > 0 else 1.0
        seasonal_factors.setdefault(combo_key, {})[cal_m] = factor
    ...
    for month in forecast_months:
        cal_m = month.month  # calendar month (7, 8, 9, ...)
        for i in range(len(combos)):
            combo_key = (combos.iloc[i]['region_role'], combos.iloc[i]['company_name'])
            sf = seasonal_factors.get(combo_key, {}).get(cal_m, 1.0)
            # Apply seasonal factor to draws for proper interval propagation
            season_draws = pred_demand[:, i] * sf
            forecasts.append({
                'cluster_name': combos.iloc[i]['region_role'],
                'company': combos.iloc[i]['company_name'],
                'forecast_month': str(month),
                'demand_predicted': max(0, round(float(point[i] * sf))),
                'demand_lower': max(0, round(float(lo[i] * sf))),
                'demand_upper': max(1, round(float(hi[i] * sf))),
                '_draws': season_draws,
            })
